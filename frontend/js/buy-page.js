class BuyPage {
    ...
    // Delegated event handling for add-to-cart and buy-now buttons inside product grid
    attachGridHandlers() {
        const grid = document.getElementById('products-grid');
        grid.addEventListener('click', (e) => {
            const addBtn = e.target.closest && e.target.closest('.add-to-cart');
            const buyBtn = e.target.closest && e.target.closest('.buy-now');
            if (addBtn) {
                const id = Number(addBtn.getAttribute('data-id'));
                this.addToCartFromGrid(id);
            }
            if (buyBtn) {
                const id = Number(buyBtn.getAttribute('data-id'));
                this.buyNowFromGrid(id);
            }
        });
    }
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card product-card h-100 shadow-sm">
                    <div class="position-relative">
                        <img src="${product.image}" class="card-img-top" alt="${product.title}" style="height: 200px; object-fit: cover;" onerror="this.onerror=null;this.src='../images/home1.jpeg';">
                        ${discount > 0 ? `
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <div class="mt-auto">
                            ...
                            <div class="d-grid gap-2">
                                <button class="btn btn-outline-secondary add-to-cart" data-id="${product.id}" type="button">
                                    <i class="bi bi-cart-plus me-2"></i>Add to Cart
                                </button>
                                <a href="product-detail.html?id=${product.id}" class="btn btn-primary">
                                    <i class="bi bi-eye me-2"></i>View Details
                                </a>
                                <button class="btn btn-warning buy-now mt-2" data-id="${product.id}" type="button">
    ...
    addToCartFromGrid(productId) {
        ...
        if (!product) return;
        const cartItem = { productId: product.id, title: product.title, price: product.price, quantity: 1 };
        try {
        ...
        // show feedback
        if (window.electroFixApp) window.electroFixApp.showNotification('Added to cart', 'success');
    }

    buyNowFromGrid(productId) {
        // For showcase: add to cart then go to cart/checkout
        this.addToCartFromGrid(productId);
